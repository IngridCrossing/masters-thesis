...
for(i in 1:length(path_options)){
  path_options[i] <- paste0("Design_Spaces/Design_Space_1/results/Design_", as.character(i), "/")
} #Paths for each iteration

source("General_Scripts/functions.R") #Source functions



designs_list <- vector(mode = "list", length = 10) #Survey designs
for (i in 1:length(designs_list)){
  designs_list[[i]] <- seq(1, i*3)
} 



det_probs <- seq(0.1, 1, 0.1) #Detection probabilities to iterate across
...
fecundity = 720 #Average number of offspring each year per breeding-age female
